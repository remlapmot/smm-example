...
stage2glm <- glm(Ystar ~ X + res, data = dat, 
                 family = Gamma('log'))
...
```{r}
dat$id <- 1:nrow(dat)
stage2geeglm <- geeglm(Ystar ~ X + res, id = id, data = dat, 
                 family = Gamma('log'))
...
# gmm fit
tsrigmmlog1 <- gmm(tsriLogMultMoments, x = dat, 
...
# gmm fit
tsrigmmlog2 <- gmm(tsriLogAddMoments, x = dat, 
                   t0 = c(coef(stage1), coef(stage2)), vcov = "iid")
...
msmm2 <- gmm(msmmAltMoments, x = dat, 
             t0 = b0[1:2], vcov = "iid")
